...
#define CATCH_CONFIG_POSIX_SIGNALS
    ...
    !defined(CATCH_INTERNAL_CONFIG_NO_CPP17_UNCAUGHT_EXCEPTIONS) && !defined(CATCH_CONFIG_CPP17_UNCAUGHT_EXCEPTIONS)
...
#define INTERNAL_CATCH_TESTCASE2(TestName, ...)                                                                        \
    ...
    namespace {                                                                                                        \
        Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                                                      \
            Catch::makeTestInvoker(&TestName), CATCH_INTERNAL_LINEINFO, "", Catch::NameAndTags{ __VA_ARGS__ });        \
    } /* NOLINT */                                                                                                     \
    CATCH_INTERNAL_UNSUPPRESS_GLOBALS_WARNINGS                                                                         \
    static void TestName()
...
#define INTERNAL_CATCH_REGISTER_TESTCASE(Function, ...)                                                                \
    ...
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(Catch::makeTestInvoker(Function),                         \
        CATCH_INTERNAL_LINEINFO, "", Catch::NameAndTags{ __VA_ARGS__ }); /* NOLINT */                                  \
    CATCH_INTERNAL_UNSUPPRESS_GLOBALS_WARNINGS
...
template <int SZ> struct StringMaker<char[SZ]> {
    static std::string convert(const char* str) { return ::Catch::Detail::stringify(std::string{ str }); }
};
...
struct SectionInfo {
    SectionInfo(
        SourceLineInfo const& _lineInfo, std::string const& _name, std::string const& _description = std::string());
...
#define INTERNAL_CHECK_THAT(macroName, matcher, resultDisposition, arg)                                                \
    do {                                                                                                               \
        ...
        INTERNAL_CATCH_TRY { catchAssertionHandler.handleExpr(Catch::makeMatchExpr(arg, matcher, #matcher)); }         \
        INTERNAL_CATCH_CATCH(catchAssertionHandler)                                                                    \
        INTERNAL_CATCH_REACT(catchAssertionHandler)                                                                    \
    } while (false)
...
namespace clara {
    namespace detail {
        ...
        template <typename ReturnType> struct LambdaInvoker {
            static_assert(
                std::is_same<ReturnType, ParserResult>::value, "Lambda must return void or clara::ParserResult");
...
Catch::MessageBuilder::MessageBuilder(
    std::string const& macroName, SourceLineInfo const& lineInfo, ResultWas::OfType type)
...
SectionEndInfo::SectionEndInfo(
    SectionInfo const& _sectionInfo, Counts const& _prevAssertions, double _durationInSeconds)
    : sectionInfo(_sectionInfo)
...
auto operator<<(std::ostream& os, StringRef const& str) -> std::ostream& { return os << str.c_str(); }
...
ConsoleReporter::ConsoleReporter(ReporterConfig const& config)
    ...
    , m_tablePrinter(new TablePrinter(config.stream(),
          { { "benchmark name", CATCH_CONFIG_CONSOLE_WIDTH - 32, ColumnInfo::Left }, { "iters", 8, ColumnInfo::Right },
...
void MultipleReporters::add(IStreamingReporterPtr&& reporter) { m_reporters.push_back(std::move(reporter)); }

ReporterPreferences MultipleReporters::getPreferences() const { return m_reporters[0]->getPreferences(); }
