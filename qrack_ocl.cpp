...
namespace Qrack {
	...
	void OCLSingleton::InitOCL(int plat, int dev) {
		...
		"	ulong otherRes, inOutRes, inOutInt, inRes, carryInt, inInt, outInt;"
		"	ulong iHigh, iLow, i;"
		"	for (lcv = ID; lcv < maxI; lcv+=Nthreads) {"
		"		iHigh = lcv;"
		"		i = 0;"
		...
		"		inOutInt = inOutRes>>inOutStart;"
		"		inRes = (lcv & inMask);"
		"		carryInt = (lcv & carryMask)>>carryIndex;"
		"		inInt = inRes>>inStart;"
		"		outInt = inOutInt + inInt + 1;"
		"		if (outInt < lengthPower) {"
		"			nStateVec[(outInt<<inOutStart) + otherRes + inRes] += stateVec[lcv + carryMask];"
		"		}"
		"		else {"
		"			nStateVec[((outInt - lengthPower)<<inOutStart) + otherRes + inRes] += stateVec[lcv + carryMask];"
		"		}"
		"	}"
		"   }"
		""
		...
		"		if (outInt < lengthPower) {"
		"			nStateVec[(outInt<<inOutStart) + otherRes + inRes] += stateVec[i + carryMask];"
		"		}"
		"		else {"
		"			nStateVec[((outInt - lengthPower)<<inOutStart) + otherRes] += stateVec[i + carryMask];"
		"		}"
		"	}"
		"   }";
