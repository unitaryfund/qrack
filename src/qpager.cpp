...
real1_f QPager::EitherCProb(bitLenInt control, bitLenInt target, bool isAnti)
{
    if (qPages.size() == 1U) {
        if (isAnti) {
            return qPages[0U]->ACProb(control, target);
        } else {
            return qPages[0U]->CProb(control, target);
        }
    }
    ...
    if ((control < qpp) && (target >= qpp)) {
        if (isAnti) {
            return QInterface::ACProb(control, target);
        } else {
            return QInterface::CProb(control, target);
        }
    }
    ...
    if (control < qpp) {
        for (bitCapIntOcl i = 0U; i < qPages.size(); ++i) {
            QEnginePtr engine = qPages[i];
#if ENABLE_PTHREAD
            futures.push_back(std::async(std::launch::async, [isAnti, engine, control, target]() {
                if (isAnti) {
...
#else
            if (isAnti) {
                oneChance += engine->AntiCProb(control, target);
            } else {
                oneChance += engine->CProb(control, target);
            }
#endif
        }
    } else {
        const bitCapIntOcl maxI = (bitCapIntOcl)qPages.size() >> ONE_BCI;
        const bitCapIntOcl cPower = isAnti ? pow2Ocl(control - qpp) : 0U;
        const bitCapIntOcl cMask = pow2Ocl(control - qpp) - ONE_BCI;
        if (target < qpp) {
            for (bitCapIntOcl i = 0U; i < maxI; ++i) {
                bitCapIntOcl j = i & cMask;
                j |= ((i ^ j) << ONE_BCI) | cPower;
                QEnginePtr engine = qPages[j];
#if ENABLE_PTHREAD
                futures.push_back(std::async(std::launch::async, [engine, target]() { return engine->Prob(target); }));
#else
                oneChance += engine->Prob(target);
#endif
            }
        } else {
            const bitCapIntOcl qPower = pow2Ocl(target - qpp);
            const bitCapIntOcl qMask = qPower - ONE_BCI;
            const bitCapIntOcl fSize = maxI >> ONE_BCI;

            const bitCapIntOcl qMask1 = (cMask < qMask) ? cMask : qMask;
            const bitCapIntOcl qMask2 = (cMask < qMask) ? qMask : cMask;

            for (bitCapIntOcl i = 0U; i < fSize; ++i) {
                bitCapIntOcl j = i & qMask1;
                bitCapIntOcl jHigh = (i ^ j) << ONE_BCI;
                bitCapIntOcl jLow = jHigh & qMask2;
                j |= jLow | ((jHigh ^ jLow) << ONE_BCI);
