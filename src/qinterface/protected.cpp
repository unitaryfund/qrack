...
void QInterface::ApplyControlledSingleBit(
...
void QInterface::ApplyControlled2x2(const bitLenInt* controls, const bitLenInt& controlLen, const bitLenInt& target,
...
void QInterface::ApplyAntiControlled2x2(const bitLenInt* controls, const bitLenInt& controlLen, const bitLenInt& target,
    const complex* mtrx, bool doCalcNorm)
{
    if (does2x2PhaseShift(mtrx)) {
        knowIsPhaseSeparable = false;
