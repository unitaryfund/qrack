...
void kernel decc(global double2* stateVec, constant ulong* ulongPtr, global double2* nStateVec)
{
    ...
    ulong skipMask =  ulongPtr[8];
    ulong partMask, oneBit, otherRes, inRes, outRes, i, iHigh, iLow;
    ulong maxCarry = lengthPower - toSub;
    for (olcv = ID; olcv < otherPower; olcv+=Nthreads) {
        partMask = skipMask;
        iHigh = olcv;
        iLow = 0;
        ...
        otherRes |= iHigh;
        for (lcv = 0; lcv < maxCarry; lcv++) {
            outRes = (lcv + maxCarry) << inOutStart;
            inRes = lcv << inOutStart;
            nStateVec[outRes | otherRes] = stateVec[inRes | otherRes];
        }
        for (lcv = maxCarry; lcv < lengthPower; lcv++) {
            outRes = (lcv - toSub) << inOutStart;
            inRes = lcv << inOutStart;
            nStateVec[outRes | otherRes | carryMask] = stateVec[inRes | otherRes];
        }
    }
