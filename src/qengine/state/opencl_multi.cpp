...
void QEngineOCLMulti::Swap(bitLenInt qubitIndex1, bitLenInt qubitIndex2) {
    ...
    if ((qubitIndex1 < subQubitCount) && (qubitIndex2 < subQubitCount)) {
        ...
        // The above commented-out logic will work in general, but doesn't receive benefit from multiple device parallelism with the current CNOT implementation. We might as well use this.
        CombineAndOp([&](QEngineOCLPtr engine) {
            engine.get()->Swap(qubitIndex1, qubitIndex2);
