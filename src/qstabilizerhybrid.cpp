...
void QStabilizerHybrid::ApplySingleBit(const complex* lMtrx, bitLenInt target)
{
    ...
    if (IS_SAME(mtrx[0], I_CMPLX * mtrx[1]) && IS_SAME(mtrx[0], -I_CMPLX * mtrx[2]) &&
...
void QStabilizerHybrid::ApplySinglePhase(const complex topLeft, const complex bottomRight, bitLenInt target)
{
    ...
    QStabilizerShardPtr gShard = std::make_shared<QStabilizerShard>(mtrx);
    shard->Compose(gShard->gate);
    shards[target] = shard;
}
    ...
    const bitLenInt& target, const complex topRight, const complex bottomLeft)
{
    ...
    } else if (IS_SAME(topRight, -I_CMPLX) && IS_SAME(bottomLeft, I_CMPLX)) {
