...
QUnitMulti::QUnitMulti(bitLenInt qBitCount, bitCapInt initState,
    qrack_rand_gen_ptr rgp, complex phaseFac, bool doNorm, bool randomGlobalPhase, bool useHostMem, int ignored,
    bool useHardwareRNG)
    : QUnit(QINTERFACE_OPENCL, qBitCount, initState, rgp, phaseFac, doNorm, randomGlobalPhase, useHostMem, -1, useHardwareRNG)
{
...
void QUnitMulti::ParFor(const bitCapInt begin, const bitCapInt end, ParallelFunc fn) {
    std::map<int, std::vector<bitLenInt>> devGroups;
    int devID;
    for (bitLenInt i = begin; i < end; i++) {
        devID = dynamic_cast<QEngineOCL*>(shards[i].unit.get())->GetDeviceID();
        if (devGroups.find(devID) == devGroups.end()) {
            devGroups[devID] = std::vector<bitLenInt>();
        }
        devGroups[devID].push_back(i);
    }
    
    par_for(0, devGroups.size(), [&](bitLenInt dev, bitLenInt cpu) {
