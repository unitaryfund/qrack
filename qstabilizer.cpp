...
// for all people, not commercialization. Dr. Aaronson, please do not hesitate to contact us at the vm6502q/qrack
// repository if you have any additional concerns.
//
...
namespace Qrack {
QStabilizer::QStabilizer(const bitLenInt& n, bool useHardwareRNG = true, qrack_rand_gen_ptr rgp = nullptr)
    : qubitCount(n)
    , rand_distribution(0.0, 1.0)
{
    ...
    bitLenInt rowCount = (n << 1U) + 1U;
    bitLenInt overBitCap = overBitCap();

    x = std::vector<std::vector<Pauli>>(rowCount);
    z = std::vector<std::vector<Pauli>>(rowCount);
    r = std::vector<char>(rowCount);

    for (bitLenInt i = 0; i < rowCount; i++) {
        x[i] = std::vector<Pauli>(overBitCap);
        z[i] = std::vector<Pauli>(overBitCap);
...
{
    bitLenInt overBitCap = overBitCap();
    for (bitLenInt j = 0; j < overBitCap; j++) {
        x[i][j] = x[k][j];
        z[i][j] = z[k][j];
    }
    r[i] = r[k];
...
char QStabilizer::clifford(const bitLenInt& i, const bitLenInt& k)
{
    bitLenInt overBitCap = overBitCap();
    Pauli pw;
...
void QStabilizer::setBasisState(const real& nrm, complex* stateVec)
{
    bitLenInt elemCount = qubitCount << 1U;
    bitLenInt j;
    bitLenInt j5;
...
complex* QStabilizer::MakeKet()
{
    ...
    complex* stateVec = new complex[pow2(qubitCount)]();
 ...
 * Measure qubit b
 */
bool QStabilizer::M(const bitLenInt& target)
{
    bitLenInt t5 = genIndex(target);
    Pauli pw = modPow2(target);
    bitLenInt elemCount = qubitCount << 1U;
    ...
    // Before, we were checking if stabilizer generators commute with Z_b; now, we're checking destabilizer generators
    for (m = 0; m < n; m++) {
        if (x[m][t5] & pw) {
            break;
        }
 ...
 */
char QStabilizer::IsSeparable(const bitLenInt& target)
{
...
bitLenInt QStabilizer::Compose(QStabilizerPtr toCopy, const bitLenInt& start)
{
    ...
    for (bitLenInt i = 0; i < nQubitCount; i++) {
        if ((i < start) || (i >= (start + length))) {
            x[i].insert(start, length, 0);
            z[i].insert(start, length, 0);
        } else {
            std::vector<Pauli> nX = toCopy->x[i - start];
            std::vector<Pauli> nZ = toCopy->x[i - start];

            nX.insert(length, qubitCount, 0);
            nZ.insert(length, qubitCount, 0);
    ...
    r.insert(start, length);
    std::copy(toCopy->r.begin(), toCopy->r.begin() + length, r.start());

    qubitCount = nQubitCount;
}

bitLenInt QStabilizer::DecomposeDispose(const bitLenInt& start, const bitLenInt& length, QStabilizerPtr toCopy)
{
    ...
    for (bitLenInt i = 0; i < qubitCount; i++) {
        if ((i < start) || (i >= end)) {
            x[i].erase(start, length);
            z[i].erase(start, length);
        } else {
            x[i].erase(start + length, endQubitCount);
            x[i].erase(0, start);
            z[i].erase(start + length, endQubitCount);
            z[i].erase(0, start);
        ...
        z.erase(start, length);
        r.erase(start, length);
