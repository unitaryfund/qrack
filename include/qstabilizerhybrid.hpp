...
public:
    ...
    virtual bool ApproxCompare(QInterfacePtr toCompare, real1_f error_tol = FP_NORM_EPSILON)
    {
        return ApproxCompare(std::dynamic_pointer_cast<QStabilizerHybrid>(toCompare), error_tol);
    }

    virtual bool ApproxCompare(QStabilizerHybridPtr toCompare, real1_f error_tol = FP_NORM_EPSILON)
    {
        FlushBuffers();
