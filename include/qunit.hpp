...
public:
    ...
    Pauli Remove(const bitLenInt& target)
    {
        Pauli qubit = generators[target];
        generators.erase(generators.begin() + target);
        return qubit;
    }

    bitLenInt Append(const Pauli& p)
    {
        bitLenInt pos = generators.size();
