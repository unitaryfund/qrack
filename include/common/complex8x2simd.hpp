...
static const __m128 SIGNMASK = _mm_castsi128_ps(_mm_set_epi32(0x80000000, 0, 0x80000000, 0));
...
struct Complex8x2Simd {
    ...
    {
        return _mm_add_ps(
            _mm_mul_ps(_mm_shuffle_ps(_val2, _val2, 180), _mm_andnot_ps(_mm_shuffle_ps(other._val2, other._val2, 165), SIGNMASK)),
...
inline Complex8x2Simd dupeLo(const Complex8x2Simd& cmplx2)
{
    return _mm_shuffle_ps(cmplx2._val2, cmplx2._val2, 80);
}
inline Complex8x2Simd dupeHi(const Complex8x2Simd& cmplx2)
{
    return _mm_shuffle_ps(cmplx2._val2, cmplx2._val2, 250);
}
inline Complex8x2Simd matrixMul(
    const Complex8x2Simd& mtrxCol1, const Complex8x2Simd& mtrxCol2, const Complex8x2Simd& qubit)
{
    __m128 dupeLo = _mm_shuffle_ps(qubit._val2, qubit._val2, 80);
    __m128 dupeHi = _mm_shuffle_ps(qubit._val2, qubit._val2, 250);
    return _mm_add_ps(_mm_add_ps(_mm_mul_ps(_mm_shuffle_ps(mtrxCol1._val2, mtrxCol1._val2, 180),
                                           _mm_andnot_ps(_mm_shuffle_ps(-dupeLo, dupeLo, 165), SIGNMASK)),
                             _mm_mul_ps(mtrxCol1._val2, _mm_shuffle_ps(dupeLo, dupeLo, 80))),
        _mm_add_ps(
            _mm_mul_ps(_mm_shuffle_ps(mtrxCol2._val2, mtrxCol2._val2, 180), _mm_andnot_ps(_mm_shuffle_ps(-dupeHi, dupeHi, 165), SIGNMASK)),
            _mm_mul_ps(mtrxCol2._val2, _mm_shuffle_ps(dupeHi, dupeHi, 80))));
}
inline Complex8x2Simd matrixMul(
    const float& nrm, const Complex8x2Simd& mtrxCol1, const Complex8x2Simd& mtrxCol2, const Complex8x2Simd& qubit)
{
    ...
    return _mm_mul_ps(_mm_set1_ps(nrm),
        _mm_add_ps(_mm_add_ps(_mm_mul_ps(_mm_shuffle_ps(mtrxCol1._val2, mtrxCol1._val2, 180),
                                        _mm_andnot_ps(_mm_shuffle_ps(-dupeLo, dupeLo, 165), SIGNMASK)),
                          _mm_mul_ps(mtrxCol1._val2, _mm_shuffle_ps(dupeLo, dupeLo, 80))),
            _mm_add_ps(_mm_mul_ps(_mm_shuffle_ps(mtrxCol2._val2, mtrxCol2._val2, 180),
                              _mm_andnot_ps(_mm_shuffle_ps(-dupeHi, dupeHi, 165), SIGNMASK)),
                _mm_mul_ps(mtrxCol2._val2, _mm_shuffle_ps(dupeHi, dupeHi, 80)))));
}
