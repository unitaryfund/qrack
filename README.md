...
Set the maximum allowed allocation (in MB) for the global pool with `QRACK_MAX_ALLOC_MB`. This includes state vectors and auxiliary buffers larger than approximately `sizeof(bitCapIntOcl) * sizeof(bitCapIntOcl)`. This should also include out-of-place single duplication of any state vector. (Note, if OpenCL resorts to evacuating from VRAM to general heap, allocation still might "explode" past the specified MB limit.)
