...
Note that this controls total direct Qrack OpenCL buffer allocation, not total Qrack library allocation. Total OpenCL buffer allocation is *not* fully indicative of total library allocation. For example, OpenCL might internally opt to migrate large buffers to different memory spaces, including as would be required for device switching. In practice, experimentally observed total system allocation might be **less** than directly tracked OpenCL allocation amounts, (if the OpenCL device provider implementation potentially uses some form of compression internally, for example).
