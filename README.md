...
`QBdt` automatically prefers the same number of "global qubits" as `QPager`, which is typically 2 qubits, to accommodate 4 maximum allocation segments on a single device. To add more qubits relative to max single page width, set the environment variable `QRACK_BDT_QB_OFFSET` to the number of additional global qubits. (If the requested number of global qubits exceeds the maximum number of qubits in a single page, all qubits will be global.) 
