...
TEST_CASE("test_weird_teleport", "[cosmos]")
{
    benchmarkLoop(
        [&](QInterfacePtr qUniverse, bitLenInt n) {
            // 0 - qubit to teleport in
            // 1 - exterior half of entering Bell pair
            // 2 - interior half of entering Bell pair
            // 3 - interior half of exiting Bell pair
            // (n - 1) - exterior half of exiting Bell pair

            // "Eve" prepares an entering Bell pair.
            qUniverse->H(1);
            qUniverse->CNOT(1, 2);

            // "Eve" prepares an exiting Bell pair.
            qUniverse->H(n - 1);
            qUniverse->CNOT(n - 1, 3);

            // Bob entangles his half of the exiting Bell pair with Alice's transformed message, at the limit of "death"
            // of his cosmogeny.
            qUniverse->CNOT(2, 3);
            qUniverse->H(2);

            // Bob measures both of his bits
            bool q0 = qUniverse->M(2);
            bool q1 = qUniverse->M(3);

            // Alice receives Bob's classical message (at the event horizon boundary between their "universes") and
            // prepares her half of the Bell pair to complete teleportation.
            if (q0) {
                qUniverse->Z(n - 1);
            }
            if (q1) {
            ...
            // "Alice" measures her received bit.
            // Exercise: After studying the entire example, quantitatively (and philosophically) describe what this bit
            // is. qUniverse->M(n - 1);

            // Bob's universe, with t->-r and r->-t runs its entire course of history (in reverse, from exterior
            // perspective).
            qUniverse->IQFT(2, n - 3);

            // "Alice" has a qubit to teleport to her past.
            real1 theta = 2 * M_PI * qUniverse->Rand();
            real1 phi = 2 * M_PI * qUniverse->Rand();
            real1 lambda = 2 * M_PI * qUniverse->Rand();
            qUniverse->U(0, theta, phi, lambda);

            // Alice entangles her half of the entering Bell pair with her "message".
            qUniverse->CNOT(0, 1);
            qUniverse->H(0);

            // Alice measures both her bits
            q0 = qUniverse->M(0);
            q1 = qUniverse->M(1);

            // "Bob" receives Alice's classical message (at the event horizon "Big Bang" "birth" limit of his
            // "universe") and prepares his half of the Bell pair to complete teleportation.
            if (q0) {
                qUniverse->Z(2);
            }
            if (q1) {
