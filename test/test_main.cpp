...
QInterfaceTestFixture::QInterfaceTestFixture()
{
    ...
    qftReg = CreateQuantumInterface({ testEngineType, testSubEngineType, testSubSubEngineType }, 10, 0, rng, ONE_CMPLX,
        enable_normalization, true, false, device_id, !disable_hardware_rng, sparse, REAL1_EPSILON, devList);
    std::dynamic_pointer_cast<QBdt>(qftReg)->Attach(std::make_shared<QStabilizer>(10, 0, rng, !disable_hardware_rng));
}
